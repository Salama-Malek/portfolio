...
export default function GooeyNav({
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(() => {
      const currentActiveLi = navRef.current && navRef.current.querySelectorAll('li')[activeIndex];
        ...
        <ul ref={navRef}>
          {items.map((item, index) => (
            <li key={index} className={activeIndex === index ? 'active' : ''}>
              <a href={item.href} onClick={(e) => { e.preventDefault(); handleClick(e, index, item.href); }} onKeyDown={(e) => handleKeyDown(e, index)}>
                {item.label}
              </a>
            </li>
          ))}
